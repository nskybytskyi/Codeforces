...
std::pair<DirectedGraph, std::vector<Component>>
BuildCondensationGraph(const DirectedGraph& directed_graph);

std::vector<int> GetRoots(const DirectedGraph& directed_graph);

void PrintAnswer(const std::vector<int>& roots,
  const std::vector<Component>& components, std::ostream& out_stream = std::cout);
...
int main() {
  ...
  const auto [condensation, components] = BuildCondensationGraph(transposed_graph);
  const auto roots = GetRoots(condensation);
  PrintAnswer(roots, components);
...
BuildCondensationGraph(const DirectedGraph& directed_graph) {
  ...
  return {condensation, components};
}
...
std::vector<int> GetRoots(const DirectedGraph& directed_graph) {
  ...
  for (const auto& adjacency_list : directed_graph) {
    for (auto adjacent : adjacency_list) {
      ++degree[adjacent];
    }
  }
  for (int vertex = 0; vertex < vertex_count; ++vertex) {
...
void PrintAnswer(const std::vector<int>& roots,
    const std::vector<Component>& components, std::ostream& out_stream) {
  out_stream << roots.size() << "\n";
  for (auto root : roots) {
    out_stream << components[root][0] + 1 << " ";
  }
  out_stream << "\n";
}
